...
export class ExcelExport {
    ...
    addTableDescription(description) {
        ...
        // Set title font to bold, and alignment to middle left
        row.eachCell(cell => {
            cell.style = {alignment: alignment.middleLeft};
        });
        this._activedRowIndex = this._activedRowIndex + 1;
    }
    ...
    addTableValues(data, noHeader) {
        data.forEach((value, index) => {
            ...
            row.eachCell({includeEmpty: true}, (cell, cellNumber) => {
                if (cell?._column?.style?.image === true) {
                    this._ws.getCell(cell._address).value = ''
                    const imageId = this._wb.addImage({
                        base64: value[cell?._column?._key],
                        extension: 'png',
                    });
                    this._ws.addImage(imageId, cell._address)
                }
            })
    ...
    createWorkSheet(data, wsSetting, wsIndex) {
        ...
        Object.keys(tableSettings).forEach(index => {
            ...
            if (tableSetting.descriptions) {
                for (let des of tableSetting.descriptions) {
                    if (des) {
                        this.addTableDescription(des);
                    }
                }
            }
